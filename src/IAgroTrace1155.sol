...
interface IAgroTrace1155 {
    ...
    enum Stage { Produccion, ProcesoEmpaque, Transporte, Llegada }
    // Estados (para dashboards)
    enum LotState { EnFinca, EnProceso, EnTransito, EnAduana, EnBodega, Entregado, Cerrado }
    ...
    event StopRecorded(uint256 indexed lotId, bytes12 placeGeohash, address actor, uint64 time, bytes32 docRefHash, bytes32 sensorRoot);
    ...
    event LotDerived(uint256 indexed parent, uint256 indexed child, uint256 amountIn, uint256 amountOut, bytes32 processType);
    ...
    function createHarvest(uint256 farmNftId, string calldata product, uint64 startDate, uint64 season, uint64 seq)
        external returns (bytes32 harvestId);
    ...
    function stageAnchor(
        uint256 id, Stage stage, uint64 time, bytes32 contentHash,
        int32 avgT, int32 lastT, int32 avgH, int32 lastH, int32 avgSoil, int32 lastSoil, bytes32 sensorRoot
    ) external;
    ...
    function processToChild(uint256 parent, uint256 child, uint256 inAmt, uint256 outAmt, bytes32 processType) external;
