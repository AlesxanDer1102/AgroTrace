...
/**
 * AgroTrace1155 — Lotes + Trazabilidad (ERC-1155)
 * - SRP: solo gestiona lotes/etapas/ruta/linaje (no roles ni certificados).
 * - OCP: extensible con nuevos procesos/etapas via eventos.
 * - LSP/ISP: usa interfaces IFarmNFT / IActorsRegistry para bajo acoplamiento.
 */
contract AgroTrace1155 is ERC1155Supply, ERC1155Burnable, AccessControl, IAgroTrace1155 {
    ...
    // roles interoperables (mismo hash que en ActorsRegistry)
    ...
    // ===== Eventos de IAgroTrace1155 ya declarados en la interfaz =====

    // ===== Constructor =====
    constructor(
        string memory baseURI,
        address farmNFT_,
        address actorsRegistry_,
        address admin
    ) ERC1155(baseURI) {
    ...
    // ===== Modificadores de autorización (vía ActorsRegistry) =====
    ...
    function createHarvest(
        ...
        uint64 season,
        uint64 seq
    ) external onlyActive(ROLE_PRODUCER) returns (bytes32 harvestId) {
    ...
    function mintLot(
        ...
        uint256 amount,
        address owner
    ) external onlyActive(ROLE_PRODUCER) {
    ...
    function setState(uint256 id, LotState s, uint64 t)
        external
    ...
    function stageAnchor(
        ...
        uint64 time,
        bytes32 contentHash,
        int32 avgT, int32 lastT, int32 avgH, int32 lastH, int32 avgSoil, int32 lastSoil,
        bytes32 sensorRoot
    ...
    function recordStop(
        ...
        bytes32 sensorRoot,
        uint64 time
    ) external onlyCustodianOrRole(ROLE_TRANSPORTER, id) {
        require(exists(id), "Trace: unknown lot");
    ...
    {
        require(exists(id), "Trace: unknown lot");
        // Reutilizamos StageAnchored para telemetría de transporte agregada por tramo (opcional)
    ...
    function declareRoute(uint256 id, bytes12 fromG, bytes12 toG, address carrier, uint64 eta)
        external onlyCustodianOrRole(ROLE_PRODUCER, id)
    {
        require(exists(id), "Trace: unknown lot");
    ...
    function markDelivered(uint256 id, bytes12 destG, uint64 t, bytes32 receiptHash)
        external onlyCustodianOrRole(ROLE_RETAILER(), id)
    {
        require(exists(id), "Trace: unknown lot");
    ...
    function splitToChild(uint256 parent, uint256 child, uint256 amount) external {
        ...
        // burn del padre en el custodio
        _burn(msg.sender, parent, amount);

        // crear hijo con mismos metadatos
    ...
    function processToChild(uint256 parent, uint256 child, uint256 inAmt, uint256 outAmt, bytes32 processType)
        external onlyActive(ROLE_PROCESSOR)
    {
        require(exists(parent), "Trace: unknown parent");
        require(lot[child].harvestId == bytes32(0), "Trace: child exists");
    ...
    // ===== Hook de transferencia para registrar paradas automáticas si viene data =====
    function _afterTokenTransfer(
        address operator, address from, address to,
        uint256[] memory ids, uint256[] memory, bytes memory data
    ) internal override(ERC1155) {
        super._afterTokenTransfer(operator, from, to, ids, new uint256, data);

        // Solo en transfer (no mint/burn) y si trae data
        if (from == address(0) || to == address(0) || data.length == 0) return;

        // data = abi.encode(bytes12 place, bytes32 docRefHash, bytes32 sensorRoot, uint64 time)
        (bytes12 place, bytes32 docRefHash, bytes32 sensorRoot, uint64 time) =
            abi.decode(data, (bytes12, bytes32, bytes32, uint64));

        for (uint256 i = 0; i < ids.length; i++) {
            emit StopRecorded(ids[i], place, operator, time, docRefHash, sensorRoot);
    ...
    // helper para acceder a harvestLots sin exponer array completo
    function harvestLots(bytes32 hId, uint256 idx) external view returns (uint256) {
        return _harvestLots[hId][idx];
    }

    // helper para hash de rol retailer (evita dependencia de contrato)
