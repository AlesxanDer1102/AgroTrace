...
/**
 * FarmNFT — Identidad de fincas en cadena (ERC-721)
 * Cada tokenId representa una finca. Guarda datos básicos y URIs.
 */
contract FarmNFT is ERC721URIStorage, AccessControl {
    ...
    struct FarmData {
        ...
        string  countryISO;   // "PE", "CO", etc.
        bytes12 geohash;      // ubicación aproximada
        string  cropFocus;    // palta/café/arándanos...
    ...
    function registerFarm(
        uint256 tokenId,
        address owner,
        FarmData calldata data,
        string calldata tokenURI_
    ) external onlyRole(FARM_MINTER_ROLE) {
        require(!_exists(tokenId), "Farm: exists");
    ...
    function updateFarm(uint256 tokenId, FarmData calldata data, string calldata newTokenURI)
        external
    {
        require(_exists(tokenId), "Farm: unknown");
        require(
            hasRole(FARM_ADMIN_ROLE, msg.sender) || ownerOf(tokenId) == msg.sender,
            "Farm: not authorized"
        );
        farm[tokenId] = data;
